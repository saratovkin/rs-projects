...
class CarItem extends React.Component {
  ...
  toggleCar = (id, status) => {
    if (!this.state.isAnimated) {
      this.loader.toggleEngine(id, status).then((res) => {
        if (status === 'started') {
          const time = res.distance / res.velocity;
          this.setState(() => {
            return { isAnimated: true, animationDuration: time };
          });
        } else {
          this.setState(() => {
            return { isAnimated: false, animationDuration: 0 };
          });
        }
      });
    }
  ...
  render() {
    const { id, name, color, isRaceStarted } = this.props;
    if (isRaceStarted) {
      this.toggleCar(id, 'started');
    }
    return (
      <li className="car-item">
        ...
        <div className="car-buttons">
          <button onClick={() => this.toggleCar(id, 'started')}>Start</button>
          <button onClick={() => this.toggleCar(id, 'stopped')}>Stop</button>
        </div>
        <svg
          style={{
            animationDuration: `${this.state.animationDuration / 1000}s`,
            animationPlayState: `${this.state.isAnimated ? 'running' : 'paused'}`,
          }}
          className={this.state.isAnimated ? 'car-icon animated' : 'car-icon'}
